...
Particularity : OpenMP parallelization of the code. No memorization of interactions between particles.
*/
    ...
    for (int i=0; i < n; i++){
        ...
        for (int j=0; j < n; j++){
            if(j != i){
                ...
                a_i -= (rP[i] + rP[j]) * kernel_1(x[i] - x[j]);
            }
        }
        // update actual output
        a[i] = a_i;
...
int main(int argc, char* argv[]){
    ...
    #pragma omp parallel
    {
        ...
        // initialize relative pressure and acceleration
        pressure(rP, x, n);
