...
From: https://github.com/pmocz/QuantumSPH/blob/master/quantumsph.py
...
def main():
  ...
  nxx = 400             # number of probe locations
  ...
  # plot potential
  xx = np.linspace(-4.0, 4.0, num=nxx)
  xx = np.reshape(xx,(xx.size,1))
  fig = plt.plot(xx, 0.5*xx**2, linewidth=5, color=[0.7, 0.7, 0.9])
  with open("results_py.csv", mode='w', newline='') as file:
    writer = csv.writer(file)
    file.write(f'n,{nxx},linewidth,{5.00},color,{0.7},{0.7},{0.9}\n')
    writer.writerows(np.concatenate((xx,0.5*xx**2),axis=1))
  ...
  for i in np.arange(-nt_setup, nt):   # negative time (t<0, i<0) is used to set up initial conditions
    ...
    if( (i >= 0) and (i % n_out) == 0 ):
      ...
      with open("results_py.csv", mode='a', newline='') as file:
        writer = csv.writer(file)
        file.write(f'n,{nxx},linewidth,{2.00},color,{0.6},{0.6},{0.6}\n')
        writer.writerows(np.concatenate((xx,rr_exact),axis=1))
        file.write(f'n,{nxx},linewidth,{2.00},color,{1.*i/nt},{0.0},{1.-1.*i/nt},label,$t={t:.2f}$\n')
        writer.writerows(np.concatenate((xx,rr),axis=1))
    ...
    if( i==-nt_setup or i==-nt_setup*3/4 or i==-nt_setup/2 ):
      ...
      with open("results_py.csv", mode='a', newline='') as file:
        writer = csv.writer(file)
        file.write(f'n,{nxx},linewidth,{1.00},color,{0.9},{0.9},{0.9}\n')
        writer.writerows(np.concatenate((xx,rr),axis=1))
